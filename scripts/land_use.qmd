...
This first code chuck handles exotic geometries present in the data. The code:
- Opens each file and seperates the exotic geometries from the standard geometries.
- Saves the exotics to their own (temporary) file and edits this exotics file using gdalUtils outside of R.
- Reads the edited exotics file back into R and combines it with the standard geometries.

Exotic geometries and those that are a bit rarer (e.g. MULTISURFACE, CURVLINEAR), whereas standard geometries are the ones everyone should be familiar with (POLYGON, LINE, POINT).
