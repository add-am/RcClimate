...
pacman::p_load(tidyverse, glue, here, janitor, sf, tmap, exactextractr, terra, RColorBrewer, ggplot2, ncdf4, lubridate)
...
#get the start date
d1 <- as.Date("1985-01-01")

#get the end date, then subtract one month (data is approximately 1 month delayed)
d2 <- floor_date(as.Date(format(Sys.time(), "%Y-%m-%d")), unit = "month") %m-% months(1)

#build a vector of YYYYMM character strings
...
file_names <- glue("{data_path}/sst_{date_vect}.nc")

#build a seoncd vector of file names for the cropped version
cropped_file_names <- glue("{data_path}/cropped_sst_{date_vect}.nc")

#build an equal length vector of url name character strings to iterate on
url_names <- glue("{base_url}{folder_vect}/ct5km_sst-mean_v3.1_{date_vect}.nc")

#map over each of the vectors, if the file does not exists locally, download it
purrr::walk2(file_names, url_names, \(x,y) {if(!file.exists(x)){download.file(y, x, mode = "wb")}})

#convert the provided sf object into a bbox
target_bbox <- n3_region |> 
  sf::st_transform("EPSG:4326") |> 
  sf::st_bbox() |> 
  st_as_sfc()

#once all files are local, crop each file
purrr::walk2(file_names, cropped_file_names \(x, y) {

  #read
  nc <- st_warp(read_stars(test), crs = "EPSG:4326")

  #crop
  nc_cropped <- sf::st_crop(nc, target_bbox)

  #save
  write_stars(nc_cropped, y)
...
for (i in n3_marine$Region) {
  ...
  cy_break <- plyr::round_any(seq(from = minmax(cy)[1], to = minmax(cy)[2], length.out = 11), 0.01)
