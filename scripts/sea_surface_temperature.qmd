...
#convert object to an terra object for exact extract to work
#test <- terra::rast(n3_sst)

#update the crs of the data to match the n3 region geometry
n3_sst <- st_warp(n3_sst, crs = "EPSG:7855")

#get mean from all layers
#sst_mean_region <- tibble(exact_extract(n3_sst, n3_marine, fun = "mean"))
...
#test <- slice(n3_sst_tbl, 1:3) |> 
  #st_as_sf()

#test_agg_tbl <- test_agg |> #st_drop_geometry() |> 
#  as.data.frame() #|> pivot_longer( cols = -geom, names_to = "time", values_to = "mean_sst" )
...
#cond_form_climate(monthly_per_wide, glue("{output_path}/sea-surface-temperature_monthly-percentiles"), indicator = "sea_surface_temperature")
...
#crop each of the layers
cy_sst_map_crop <- st_crop(cy_sst_map, n3_marine)
anom_sst_map_crop <- st_crop(anom_sst_map, n3_marine)
...
#for (i in 1991:2020) {
  ...
  #select only the current financial year from the reference table
  #target_period <- name_date_tbl |> filter(Fyear == i)

  #return the index of each layer that shares a date with the target table
  #layer_index <- as.vector(which(st_get_dimension_values(n3_sst, "time") %in% as.Date(target_period$LayerDate)))

  #extract those layers and then create a mean of current fyear map
  #cy_sst_map <- n3_sst[,,,layer_index] |> 
...
#extract the results into a simple table
#all_anom_tbl <- bind_rows(map(all_anom_layers, as_tibble))

#then retreive a min and max value
#anom_min_val <- min(all_anom_tbl$cropped_sst_198501.nc, na.rm = TRUE)
#anom_max_val <- max(all_anom_tbl$cropped_sst_198501.nc, na.rm = TRUE)


#test <- as_tibble(all_anom_maps[[1]])
...
if (script_fac > 1) {
  ...
  #create the new resolution object and warp, all in the same move as a custom func
  dis_agg_fnc <- function(x){   
    st_warp(x, st_as_stars(
      st_bbox(x), 
      dx = (1/script_fac) * st_dimensions(x)$x$delta, 
      dy = (1/script_fac) * st_dimensions(x)$y$delta
    ), method = "bilinear", use_gdal = TRUE)
  }

  #run the function
  cy_sst_map_crop <- dis_agg_fnc(cy_sst_map_crop[,,,, drop = TRUE])
  anom_sst_map_crop <- dis_agg_fnc(anom_sst_map_crop[,,,, drop = TRUE])
  sst_all <- dis_agg_fnc(sst_all)

  #then mask again to get a precise border
  cy_sst_map_crop <- st_crop(cy_sst_map_crop, n3_marine)
  anom_sst_map_crop <- st_crop(anom_sst_map_crop, n3_marine)
  sst_all <- st_crop(sst_all, n3_marine)
}
