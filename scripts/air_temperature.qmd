...
output_path <- here(glue("outputs/air_temperature/{script_year}/"))
...
tsv <- st_as_sf(data.frame(Place = "Townsville", x = "-19.2590", y = "146.8169"), coords = c("y", "x"), crs = script_crs)
...
min_tmp <- glue("{data_path}/raw/tmax2")

#create a path to the cropped min and map temperature folders
min_tmp <- glue("{data_path}/processed/tmin2")
min_tmp <- glue("{data_path}/processed/tmax2")

#create a path to where the final dataset should be saved
final <- glue("Unsure atm")

#---- inside func now?

#create a list of all min and max files
file_list_min <- list.files(min_tmp, full.names = TRUE, recursive = TRUE)
file_list_max <- list.files(max_tmp, full.names = TRUE, recursive = TRUE)

#some kind of safety check for extension type
#..

#read in all data
at_min <- stars::read_stars(file_list_min)
at_max <- stars::read_stars(file_list_max)

#create a path to the finished dataset
path1 <- glue("{data_path}/n3_tmean_temperature.nc")
...
if (file.exists(path1)) {#if the full cut down version exists open that
  ...
  file_list_max <- list.files(glue("{data_path}/tmax2"), full.names = TRUE, recursive = TRUE)
  file_list_min <- list.files(glue("{data_path}/tmin2"), full.names = TRUE, recursive = TRUE)

  #drop files with extension ".md" they are just notes
  file_list_max <- file_list_max[!str_detect(file_list_max, "note.md")]
  file_list_min <- file_list_min[!str_detect(file_list_min, "note.md")]

  test_max <- read_stars(file_list_max) |> 
    st_warp( crs = script_crs) |> 
    st_crop(st_union(qld))

  test_min <- read_stars(file_list_min) |> 
    st_warp( crs = script_crs) |> 
    st_crop(st_union(qld))

  test_mean <- c(test_max, test_min, along = 3) 
  
  test_mean <- st_apply(test_mean, c("x", "y"), mean, na.rm = TRUE)
  ...
  #update crs info
  crs(max_tmp) <- script_crs
  crs(min_tmp) <- script_crs
...
write_csv(summary_tbl, glue("{output_path}/air-temperature_summary.csv"))
...
save_n3_table(
  df = monthly_per_wide,
  file_name = glue("{output_path}/air-temperature_monthly-percentiles"),
...
#dplyr::select only the current financial year
...
for (i in 1911:1940) {
  ...
  #Use subset to dplyr::select layers and take the mean into a single layer
...
for (i in c((1911:1940), (2019:pmax(2023, current_fyear)))){
  ...
  #Use subset to dplyr::select layers and sum into a single layer
  raster_layer <- mean(subset(n3_tmean, target_period$LayerName))
