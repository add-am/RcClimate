...
#cut down the n3 dataset to marine region
...
#cut down the n3 dataset to everything but the marine region
n3_basins <- n3_region |> 
  name_cleaning() |> 
  filter(Environment != "Marine") |> 
...
#path to files
path1 <- glue("{data_path}/n3_dhw.nc")
...
if (file.exists(path1)) {#if the full cut down version exists open that
  ...
  dowload_path <- glue("{data_path}/dhw_world/")
