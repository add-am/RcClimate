...
pacman::p_load(glue, here, sf, tmap, exactextractr, terra, RColorBrewer, ggplot2, openxlsx2, stars)
...
#define the targeted financial year
current_fyear <- 2025

#build some paths and folders
data_path <- here("data/")
...
#if the data exists on file, read it in
if (any(file.exists("n3_region.gpkg", data_path))){

  n3_region <- st_read(glue("{data_path}/n3_region.gpkg"))

#otherwise, build it using the function and save to file
} else {
  
  n3_region <- build_n3_region()

  st_write(n3_region, glue("{data_path}/n3_region.gpkg"))
}
...
#define the name of the dataset to be saved on file (or read back in)
data_name <- "n3_rainfall.nc"

#check if the data exists on file, if it does read it in
if (data_name %in% list.files(glue("{data_path}"))){

  rain_data <- st_read(glue("{data_path}/data_name"))

#otherwise use the custom function to extract the data from the API
} else {

  rain_data <- rainfall_extract(n3_region)

  write_stars(rain_data, glue("{data_path}/data_name"))
}
...
cond_form_climate(monthly_per_wide, glue("{output_path}/rainfall_monthly-percentiles"), indicator = "rainfall")
