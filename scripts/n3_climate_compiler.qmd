...
pacman::p_load(tidyverse, glue, here, janitor, readxl, huxtable)

#load in the custom function used to create the read and write folders for the script
source(here("functions/script_setup.R"))

#run the function to create the folders and paths (we are suppressing warnings so it executes silently)
suppressWarnings(script_setup())
...
#set base paths to each of the data folders (the base script set up function hasn't done this)
rainfall_base_path <- here(glue("outputs/02. northern_three/n3_climate_rainfall/{current_fyear}"))
temperature_base_path <- here(glue("outputs/02. northern_three/n3_climate_air-temperature/{current_fyear}"))
sst_base_path <- here(glue("outputs/02. northern_three/n3_climate_sea-surface-temperature/{current_fyear}"))
dhw_base_path <- here(glue("outputs/02. northern_three/n3_climate_dhw/{current_fyear}"))
landuse_base_path <- here(glue("outputs/02. northern_three/n3_climate_land-use/"))
...
rainfall_annual_summary <- rainfall_annual |> 
  dplyr::select(Region, Basin, AnnualMeanRainfall, AnnualLtm) |> 
  unique()
...
temperature_annual_summary <- temperature_annual |> 
  dplyr::select(Region, Basin, AnnualMeanTemperature, AnnualLtm) |> 
  unique()
...
sst_annual_summary <- sst_annual |> 
  dplyr::select(Region, AnnualMeanSst, AnnualLtm) |> 
  unique()
...
lu_table_present <- landuse_annual |>
  dplyr::select(Basin, Landuse, LanduseKm2, 
...
rainfall_annual_ltm <- rainfall_annual |> 
  dplyr::select(Basin, AnnualMeanRainfall, AnnualLtm, annual_anomaly, annual_percentage_of_ltm) |> 
...
temperature_annual_ltm <- temperature_annual |> 
  dplyr::select(Basin, AnnualMeanTemperature, AnnualLtm, AnnualAnomaly, AnnualPercentageOfLtm) |> 
...
sst_annual_ltm <- sst_annual |> 
  dplyr::select(Region, AnnualMeanSst, AnnualLtm, AnnualAnomaly, AnnualPercentageOfLtm) |> 
